...
Output: scratchpad/rh_factory/candidates_sl1.jsonl.gz (one row per candidate).
"""
import bisect
import collections
import gzip
...
CLASSES = {
    ...
    # SL1 arms (2026-07-17 dollar-conversion fix): identical to their parent
    # ladder + the loss-side mirror of TP1 — first touch of -6% pre-TP1 banks
    # 0.75; only the tail rides to the stop. Booked at the OBSERVED price
    # (no favorable slip on a loss leg). Paired per-candidate vs the parent.
    "scalp_sl1": dict(tp1=6.0, f1=0.75, tp2=12.0, f2=0.25, stop=-15.0,
                      trail=3.0, prearm=5.0, pregap=2.0, tbox=None,
                      sl1=-6.0, fs1=0.75),
    "aged_sl1":  dict(tp1=6.0, f1=0.50, tp2=16.0, f2=0.30, stop=-15.0,
                      trail=10.0, prearm=5.0, pregap=2.0, tbox=None,
                      sl1=-6.0, fs1=0.75),
}
...
def ladder_step(c, s, pnl, dt):
    ...
    if c["tbox"] is not None and dt >= c["tbox"]:
        close_all(); return
    if not s["tp1"]:
        # SL1 loss-ladder (mirror of the lane's lane-side SL1): first touch of
        # sl1 pre-TP1 banks fs1 at the OBSERVED price; the tail rides on.
        if (c.get("sl1") is not None and not s["sl1f"] and pnl <= c["sl1"]):
            f = min(c["fs1"], s["rem"])
            s["real"] += f * pnl
            s["rem"] -= f
            s["sl1f"] = True
...
gz_out = gzip.open(os.path.join(OUT, "candidates_sl1.jsonl.gz"), "wt",
                   encoding="utf-8")
