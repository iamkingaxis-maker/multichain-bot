...
async def main():
    ...
    async def _gc_tune():
        ...
        # ONE-TIME HEAP HISTOGRAM — MUST run BEFORE gc.freeze() (get_objects()
        # excludes the frozen permanent gen, so post-freeze it sees ~nothing).
        # Finds the retention hog behind the ~4.5M-object heap / memory bill.
        try:
    ...
    if config.enable_solana:
        ...
        sol_smart_follow = SmartMoneyFollowStrategy(
            ...
            # 2026-06-08 anti-spray: k=3 bought 28 distinct tokens in 30 fires. k=4 ->
            ...
            # 2026-06-09: K=1 (option-3 pivot) flooded the event loop — every buy by 3 active
            # wallets fired process_external_signal, starved the async HTTP server (dashboard
