...
class Config:
    ...
    dip_tp1_sell: float = 0.33            # 33% of original (runner-tilt 2026-05-17 PM, was 0.50). Per-cohort exit-ladder mining showed current (50%/25%/25% trail-1pp) is the WORST ladder across all 6 trigger cohorts. runner-tilt strictly dominates: same WR, +4-6pp/trade.
    dip_tp2_pct: float = 10.0             # TP2 at +10% (runner-tilt 2026-05-17 PM, was 5.0). Wider TP2 captures the +10-20% peak bucket (20% of universe events) instead of clipping at +5%.
    dip_tp2_sell: float = 0.50            # 50% of remainder (= 33.5% of original given new TP1=0.33). Trail gets ~34% of original.
    ...
    dip_stop_pct: float = 15.0            # Hard stop at -15% (was -4%). LOOSENED 2026-05-17 PM per user direction. Rationale: today's mining showed memecoin runners (fresh_runner_factory: 44% hit +50%+) often have deep early drawdowns before exploding. Tight -4% stop killed runners-in-development. Trade-off: each stopped trade now loses 15% × $20 = $3.00 (vs $0.80 at -4%). Requires winners to capture larger peaks to maintain EV — the runner-tilt exit ladder is designed for this. Watch closely forward: if WR drops + avg loss size dominates, revert toward -7%.
    dip_winner_trail_pct: float = 3.0     # Widened 1.0 → 3.0 on 2026-05-17 PM (runner-tilt). Per-cohort exit ladder mining showed wider trail lets runners run on the 34% trail allocation. Captures the +20-50%+ peaks current 1pp trail kept clipping. Cost: each trail exit gives back 3pp instead of 1pp. Net positive because peak distribution is right-tailed (universe: 19% of events peak +20-50%, 7% peak +50%+).
