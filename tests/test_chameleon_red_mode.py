...
def _green_config():
    return types.SimpleNamespace(
        ...
        mcap_min=None,   # off-red the chameleon has no mcap floor; red mode adds one
    )
...
def test_apply_red_profile_swaps_entry_triggers_geometry():
    ...
    assert "shape_90m_drawdown_from_max_pct" in gate_feats                          # deep-dip ADDED
    assert "1m_volume_spike" not in gate_feats                                      # vol gate DROPPED (it picked losers)
    ...
    assert c.mcap_min == 50000.0                                                    # mcap floor ADDED (skip micro losers)
...
def test_restore_green_is_exact():
    ...
    assert c.mcap_min is None                               # mcap floor lifted (restored to None, not stuck on)
