...
def main():
    ...
    # build closed trips: per token, first buy ts + realized ret%
    trips = []   # (mint, entry_ts, ret%)
    ...
    json.dump(feats, open(os.path.join(os.path.dirname(HERE), "_8zkg_entry_feats.json"), "w"))
    print(f"\nreconstructed {len(feats)}/{len(trips)} entries -> _8zkg_entry_feats.json")

    wins = [f for f in feats if f["ret"] > 0]
    losses = [f for f in feats if f["ret"] <= 0]
    print(f"\n=== 8zkg ENTRY-STATE: WINNERS vs LOSERS  (realized) ===")
